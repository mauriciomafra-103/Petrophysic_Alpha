...
def RegressaoRiosTreino(Dados_Rios, N_permeabilidade = 'Permeabilidade Gas', Log = True, N_componentes = 6,
                  Distribuicao_tempo = True, C_distribuicao = ['T2 0.01',  'T2 0.011',  'T2 0.012',  'T2 0.014',  'T2 0.015',  'T2 0.017',  'T2 0.019',  'T2 0.021',  'T2 0.024',
           'T2 0.027',  'T2 0.03',  'T2 0.033',  'T2 0.037',  'T2 0.041',  'T2 0.046',  'T2 0.051',  'T2 0.057',  'T2 0.064',
           'T2 0.071',  'T2 0.079',  'T2 0.088',  'T2 0.098',  'T2 0.109',  'T2 0.122',  'T2 0.136',  'T2 0.152',  'T2 0.169',
           'T2 0.189',  'T2 0.21',  'T2 0.234',  'T2 0.261',  'T2 0.291',  'T2 0.325',  'T2 0.362',  'T2 0.404',  'T2 0.45',
           'T2 0.502',  'T2 0.56',  'T2 0.624',  'T2 0.696',  'T2 0.776',  'T2 0.865',  'T2 0.964',  'T2 1.075',  'T2 1.199',
           ...
           'T2 1265.792',  'T2 1411.258',  'T2 1573.441',  'T2 1754.262',  'T2 1955.864',  'T2 2180.633',  'T2 2431.234',  'T2 2710.634',  'T2 3022.143',
           'T2 3369.45',  'T2 3756.671',  'T2 4188.391',  'T2 4669.725',  'T2 5206.375',  'T2 5804.697',  'T2 6471.778',  'T2 7215.521',  'T2 8044.736',
           'T2 8969.245',  'T2 10000.0'], Nome_distribuicao = 'Distribuicao T2 Thikonov Suave'):
    ...
    if Distribuicao_tempo == True:
      X_treino = Dados_Rios[C_distribuicao]

    else:
      X_treino = Dados_Rios[Nome_distribuicao]

    if Log == True:

      y_treino = np.log10(Dados_Rios[N_permeabilidade]*1000)
      ...
      #Erro Sigma
    k_p_treino = np.log10(Dados_Rios['Permeabilidade Prevista Rios'])
    k_g_treino = np.log10(Dados_Rios[N_permeabilidade])
    N = len(k_p_treino)
    soma = np.sum((k_p_treino-k_g_treino)**2)
    raiz = np.sqrt(soma/N)
    sigma_treino = 10**(raiz)

    return plsr, Dados_Rios, sigma_treino
